...
<head>
	...
	<?php } else if ($mask == 0) { ?>
	<link rel="stylesheet" href="onScreen2.css">
	<?php  } else { ?>
	<link rel="stylesheet" href="onScreen2Mask.css">
	...
	function updateTime(){
		...
		if (matchActive){
			...
			if (msToEnd > matchTime * 1000) {
				...
				if (seconds == 0){
					...
					seconds = seconds;
					$(".countDown").show();
					$(".leftBotCountdown").show();
					$(".rightBotCountdown").show();
				} 
				clockText = seconds;
				...
				$(".countDown").hide();
				$(".leftBotCountdown").hide();
				$(".rightBotCountdown").hide();
			} else {
		...
		else if (msToEnd < 0 && msToEnd > -10000){
	...
	function updateCage(cage){
		  $.get('matchFunctions.php?mode=cageJson&cage=<?php echo $cage; ?>', function( data ) { 
			  ...
			  $(".centerBotImageLeft").css("background-image", "url('robots/" + cage.player1.replaceAll(' ', '').toLowerCase() + ".jpg')");
			  ...
			  $(".centerBotImageRight").css("background-image", "url('robots/" + cage.player2.replaceAll(' ', '').toLowerCase() + ".jpg')");
			  ...
			  if (cage.player1 == cage.winner || cage.player2 == cage.winner){
				  playerHasWon = true;
				  $('.countDownTime').html(cage.winner+" Wins!");
			  } else {
				  playerHasWon = false;
			  }
...
<div class="countDown">
	<div class="leftBotCountdown">
		<div class="centerBotName" id="player1countdown"></div>
		<div class="centerBotImageLeft" style="background-image:url('robots/<?=$player1image?>.jpg')"></div>
	</div>
	<div class="rightBotCountdown">
		<div class="centerBotName" id="player2countdown"></div>
		<div class="centerBotImageRight" style="background-image:url('robots/<?=$player2image?>.jpg')"></div>
	</div>
